...
namespace EyeProtect.Core.Cache.Internal
{
    ...
    public class Cache<TData> : CacheBase, ICache<TData>
    {
        ...
        public virtual async Task<bool> ExistsAsync(string key)
        {
            var cacheKey = GetCacheKey(key);
            if (StoragePolicy.HasFlag(CacheStoragePolicy.Memory) &&
                MemoryCache.TryGetValue(cacheKey, out var _))
            {
                return true;
            }
            ...
            if (StoragePolicy.HasFlag(CacheStoragePolicy.Redis))
            {
                return await RedisExistsAsync(cacheKey);
            }
        ...
        /// <summary>
        /// 检查Redis是否存在指定 <paramref name="cacheKey"/> 缓存
        /// </summary>
        protected virtual async Task<bool> RedisExistsAsync(string cacheKey)
        {
            return await RedisCache.KeyExistsAsync(cacheKey);
        }

        /// <summary>
        /// 检查Redis是否存在指定 <paramref name="cacheKey"/> 缓存
        /// </summary>
        protected virtual bool RedisExists(string cacheKey)
        {
            return RedisCache.KeyExists(cacheKey);
        }
