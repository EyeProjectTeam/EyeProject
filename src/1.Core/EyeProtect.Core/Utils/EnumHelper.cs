...
namespace EyeProtect.Core.Utils
{
    ...
    public static class EnumHelper
    {
        ...
        /// 新增获取枚举显示名和值
        /// </summary>
        public static Dictionary<string, int> GetEnumDisplayNameValues(this Type type)
        {
            ...
            foreach (var value in Enum.GetValues(type))
            {
                ...
                var displayAttribute = enumType.GetField(name).GetCustomAttribute<DisplayAttribute>(false);

                if (displayAttribute == null)
                    continue;

                if (!displayAttribute.Name.IsNullOrWhiteSpace())
                    result.Add(displayAttribute.Name, Convert.ToInt32(value));
            }
